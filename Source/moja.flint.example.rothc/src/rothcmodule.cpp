...
void RothCModule::ComputeTSMDParams(double clayFrac, double sampleDepth, double bareToCvrdRatio) {
   ...
   if (!(maxTSMDCvrd > MathEx::k0Plus && maxTSMDBare > MathEx::k0Plus)) {  // ie assert(maxTSMD > k0Plus)
      const auto str = fmt::format("maxTSMDCvrd ({}) && maxTSMDBare ({}) > k0Plus", maxTSMDCvrd, maxTSMDBare);
      BOOST_THROW_EXCEPTION(flint::SimulationError()
                            << flint::Details(str) << flint::LibraryName("moja.modules.fullcam")
                            << flint::ModuleName("RothCModule::ComputeTSMDParams") << flint::ErrorCode(2));
   }
}
...
void RothCModule::InitializeForASimulation() {
   ...
   operation->addTransfer(_initialValues, _soilDPM, initSoil["dpmaCMInit"])
...
void RothCModule::UpdateRateProperties() {
   ...
   TSMD = std::max(0.0, netWaterInput >= 0        ? TSMD - netWaterInput
                        : _isSoilCovered->value() ? std::min(maxTSMDCvrd, TSMD - netWaterInput)
                        : TSMD > maxTSMDBare      ? TSMD
                                                  : std::min(maxTSMDBare, TSMD - netWaterInput));
...
void RothCModule::SubmitMoves() {
   ...
   operation->addTransfer(_soilDPM, _atmosphere, dpmSdcmFrac * fracSdcmToAtms)
...
void RothCModule::onTimingInit() { InitializeForASimulation(); }
